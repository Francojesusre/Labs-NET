...
namespace Lab04
{
    ...
    {
        protected string connectionString { get { return "server=serverisi;database=net;uid=net;pwd=net;"; } }

        public override DataTable getTabla()
        {
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                MySqlCommand cmdSelect = new MySqlCommand("select * from costactos", conn);
                conn.Open();
